public class TestRunner {
    ...
    public static void testStep2() {
        ...
        assert (buffer[10] & 0xFF) == 0x80 : "bit de poids fort incorrect";
        assert buffer[11] == 0 && buffer[12] == 0 && buffer[13] == 0;
        assert Utils.readInt(buffer, 10) == 0x80000000 : "Erreur avec 0x80000000";
        assert Utils.readInt(buffer, 10) == Integer.MIN_VALUE;

        Utils.writeInt(buffer, 10, -1);
        assert Utils.readInt(buffer, 10) == -1;
    ...
    public static void testStep3() {
        ...
        Utils.writeLong(buffer, 40, t);
        assert Utils.readLong(buffer, 40) == t : "Erreur timestamp";
        Utils.writeLong(buffer, 40, 0x8000000000000000L);
        assert Utils.readLong(buffer, 40) == Long.MIN_VALUE;
    ...
    public static void testStep4() {
        ...
        for (int i = 0; i < 16; i++) {
            assert (mem[MemoryManager.BITMAP_OFFSET + i] & 0xFF) == 0xFF :
                    "Blocs systeme non reserves (octet " + i + ")";
        }
        assert (mem[MemoryManager.BITMAP_OFFSET + 16] & 0xFF) == 0x01 :
                "Le bloc 128 doit etre reserve et pas le 129";
